  ...
  <svg xmlns="http://www.w3.org/2000/svg" width="900" height="640" viewBox="0 0 900 640">
    <defs>
      <linearGradient id="bg" x1="0" y1="0" x2="1" y2="1">
        ...
        <stop offset="45%" stop-color="#e0ece4"/>
...
export default function VehicleCard({ vehicle, onDetails, onWishlist, wished = false }) {
  return (
    <motion.article
      ...
      className="premium-card bg-[#fbf7f0] tilt-anchor group overflow-hidden"
    >
      <div>
        <div className="relative aspect-[4/3] overflow-hidden bg-white">
          <img
            ...
            className="h-full w-full object-contain p-3 transition-transform duration-500 ease-out will-change-transform group-hover:scale-105"
          />
          <div className="absolute left-4 top-4 rounded-full border border-[#d8d3cd] bg-[#fbf7f0]/95 px-3 py-1 text-xs font-semibold text-slate-700">
            {vehicle.type}
          </div>
          <button
            onClick={onWishlist}
            className={`absolute right-4 top-4 inline-flex h-10 w-10 items-center justify-center rounded-full border ${
              wished ? 'border-brand-blue bg-brand-blue text-white' : 'border-[#d8d3cd] bg-[#fbf7f0]/95 text-slate-800'
            } shadow-lg`}
            aria-label="Wishlist"
          >
        ...
        <div className="space-y-5 p-6">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="rounded-2xl bg-brand-gray px-3 py-2 text-right text-white">
              <div className="text-[10px] uppercase tracking-[0.25em] text-slate-300">Price</div>
          ...
          <div className="grid grid-cols-3 gap-3 text-sm">
            {[
              ...
              <div key={k} className="rounded-2xl bg-brand-soft p-3">
          ...
          <button
            onClick={onDetails}
            className="magnetic inline-flex w-full items-center justify-center gap-2 rounded-full bg-brand-gray px-5 py-3 text-sm font-semibold text-white transition hover:bg-brand-blue"
          >
            View Details
